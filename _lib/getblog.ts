...
export async function getBlogs(search:string ="", page:number=1) {
  ...
  const res = await fetch(` https://f0c4-82-212-85-210.ngrok-free.app/blog-articles?search_term=${search}&page=${page}`);
...
export async function getBlog(id: number) {
  ...
  const res = await fetch(` https://f0c4-82-212-85-210.ngrok-free.app/blog-articles/${id}`);
